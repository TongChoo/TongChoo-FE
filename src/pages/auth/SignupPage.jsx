...
function validateSignupForm(form) {
  ...
  else if (!form.email.includes("@")) fieldErrors.email = "올바른 이메일 형식으로 입력해주세요.";
  ...
  else if (form.nickname.trim().length > 20) fieldErrors.nickname = "닉네임은 20자 이하여야 합니다.";
...
export default function SignupPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      } else if (error.status === 409 && error.message.includes("이메일")) {
        setFieldErrors({ email: error.message });
      } else if (error.status === 409 && error.message.includes("닉네임")) {
    ...
    <main className="min-h-[calc(100vh-90px)] flex items-center justify-center px-6 py-16 bg-white">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-5 text-3xl font-bold text-navy-950">회원가입</h1>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          <div>
            ...
            />
            {fieldErrors.email && <p role="alert" className="mt-1.5 text-sm font-medium text-danger-text">{fieldErrors.email}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="mt-1.5 w-full rounded-md border border-border-input px-3.5 py-2.5 text-sm text-navy-900 placeholder:text-[#a3b2c7] focus:outline-none focus:border-brand-primary transition"
            />
            {fieldErrors.password && <p role="alert" className="mt-1.5 text-sm font-medium text-danger-text">{fieldErrors.password}</p>}
          </div>

          <div>
            <label htmlFor="nickname" className="block text-sm font-bold text-navy-700">닉네임</label>
            <input
              type="text"
              id="nickname"
              name="nickname"
              ...
              className="mt-1.5 w-full rounded-md border border-border-input px-3.5 py-2.5 text-sm text-navy-900 placeholder:text-[#a3b2c7] focus:outline-none focus:border-brand-primary transition"
            />
            {fieldErrors.nickname && <p role="alert" className="mt-1.5 text-sm font-medium text-danger-text">{fieldErrors.nickname}</p>}
          </div>
