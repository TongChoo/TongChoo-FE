...
function AftermathBadge({ days }) {
  if (days === null || days === undefined) {
    return <span className="text-sm font-normal text-navy-300">&mdash;</span>;
  }
  ...
  return (
    <span
      className={[
        "px-2.5 py-1 text-sm font-bold rounded-md whitespace-nowrap",
        isSoon ? "text-suspicion-high-text bg-suspicion-high-bg" : "text-brand-primary bg-brand-primary-soft",
      ].join(" ")}
    >
      {label}
    </span>
...
export default function ExcuseHistoryPage() {
  ...
  useEffect(() => {
    ...
    async function loadExcuses() {
      try {
        ...
        if (isMounted) {
          setErrorMessage(error.message || "변명 기록을 불러오지 못했습니다.");
        }
      } finally {
  ...
  const visibleExcuses = useMemo(() => {
    const filtered = excusePage.content.filter((excuse) =>
      (excuse.situation ?? "").toLowerCase().includes(query.trim().toLowerCase()),
    );

    return filtered.sort((a, b) => {
      if (sortBy === "aftermath") {
        const daysA = a.nextAftermathInDays === null || a.nextAftermathInDays === undefined ? Infinity : Number(a.nextAftermathInDays);
        const daysB = b.nextAftermathInDays === null || b.nextAftermathInDays === undefined ? Infinity : Number(b.nextAftermathInDays);
        return daysA - daysB;
      }
    ...
    <main className="max-w-7xl mx-auto px-6 py-12 bg-white">
      <div className="flex items-center justify-between">
        ...
        <p className="text-sm font-normal text-navy-500">
          총 <span className="font-bold text-navy-900">{excusePage.totalElements}</span>개
        </p>
      </div>

      <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          <svg viewBox="0 0 24 24" className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#a3b2c7]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <circle cx="11" cy="11" r="7" />
            <path d="m21 21-4.3-4.3" />
          </svg>
          <label htmlFor="search-input" className="sr-only">상황으로 검색</label>
          <input
            type="search"
            id="search-input"
            placeholder="상황으로 검색"
        ...
        <div className="relative flex items-center gap-2 shrink-0">
          <span className="text-sm font-medium text-navy-500 whitespace-nowrap">정렬</span>
          ...
          <table className="w-full min-w-[760px] text-left">
            <thead>
              <tr className="text-xs font-normal text-navy-300 border-b border-border-soft">
                <th scope="col" className="px-6 py-4 font-normal">상황</th>
                <th scope="col" className="px-4 py-4 font-normal">상대 · 강도</th>
                <th scope="col" className="px-4 py-4 font-normal">성공 확률</th>
                <th scope="col" className="px-4 py-4 font-normal">후폭풍</th>
                <th scope="col" className="px-4 py-4 font-normal">생성 시각</th>
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-sm font-normal text-navy-300">
                    변명 기록을 불러오는 중...
                  </td>
                </tr>
                ...
                visibleExcuses.map((excuse, index) => (
                  <tr
                    key={excuse.id}
                    onClick={() => navigate(`/excuses/${excuse.id}`)}
                    className={[
                      "hover:bg-surface-soft transition-colors cursor-pointer",
                      index !== visibleExcuses.length - 1 ? "border-b border-border-soft" : "",
                    ].join(" ")}
                  >
                    ...
                    <td className="px-4 py-6">
                      <span className="px-2.5 py-1 text-sm font-medium text-brand-primary bg-brand-primary-soft rounded-md whitespace-nowrap">
                    ...
                    <td className="px-4 py-6">
                      <AftermathBadge days={excuse.nextAftermathInDays} />
                    </td>
                    <td className="px-4 py-6 text-sm font-normal text-navy-300 whitespace-nowrap">
                      {formatDateTime(excuse.createdAt)}
                    </td>
                    <td className="px-6 py-6 text-right">
                      <svg viewBox="0 0 24 24" className="w-5 h-5 ml-auto text-[#a3b2c7]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                        <path d="M9 6l6 6-6 6" />
                      </svg>
                    </td>
                  </tr>
      ...
      <nav aria-label="페이지네이션" className="mt-6 flex items-center justify-center gap-4">
        <button
          type="button"
