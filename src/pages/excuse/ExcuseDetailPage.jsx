...
function DotList({ items, color = "bg-brand-primary", textColor = "text-navy-700", emptyText }) {
  if (!items?.length) {
  ...
  return (
    <ul className="space-y-2">
      {items.map((item) => (
        <li key={item} className={`flex items-start gap-2 text-sm font-normal ${textColor}`}>
...
export default function ExcuseDetailPage() {
  ...
  useEffect(() => {
    ...
    async function loadExcuse() {
      try {
        ...
        if (isMounted) {
          setErrorMessage(error.message || "변명 상세를 불러오지 못했습니다.");
        }
      } finally {
  ...
  if (isLoading) {
    return (
      <main className="max-w-7xl mx-auto px-6 py-12 bg-white">
        <p className="text-sm font-medium text-navy-500">변명 상세를 불러오는 중...</p>
      </main>
    );
  }
  ...
  if (errorMessage || !excuse) {
    return (
      <main className="max-w-7xl mx-auto px-6 py-12 bg-white">
        <Link to="/excuses" className="inline-flex items-center gap-1.5 text-sm font-medium text-navy-500 hover:text-brand-primary transition-colors">
    ...
    <main className="max-w-7xl mx-auto px-6 py-12 bg-white">
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-6 items-start">
        ...
        <div className="border border-border-soft rounded-lg">
          ...
          <div className="p-6 sm:p-8 border-t border-border-soft">
            ...
            <form onSubmit={handleEvolveSubmit} className="mt-3 flex flex-col gap-3">
              <input type="hidden" name="excuseId" value={excuse.id} />
      ...
      <section aria-label="대화 스레드" className="mt-6 border border-border-soft rounded-lg p-6 sm:p-8">
        ...
        <div className="mt-5">
          <button
            type="button"
            onClick={() => setNotice("답장 준비 기능은 다음 단계에서 API와 연결할게요.")}
